...
Usage:
    python3 scripts/convert-to-latex.py <reports_dir>

Example:
    python3 scripts/convert-to-latex.py docs/reports/aws-multiuser-cli-1qemu/
"""
import os
import re
import sys
...
def main():
    if len(sys.argv) < 2:
        print(f"Uso: python3 {sys.argv[0]} <reports_dir>", file=sys.stderr)
        print(f"Ej:  python3 {sys.argv[0]} docs/reports/aws-multiuser-cli-1qemu/", file=sys.stderr)
        sys.exit(1)
    ...
    if not html_files:
        print(f"No se encontraron archivos .html en '{base_dir}'.")
        sys.exit(0)

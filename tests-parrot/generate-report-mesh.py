...
<title>Reporte Mesh CLI Multiusuario \u2014 LinkGuard v2</title>
<style>
...
<body>
<h1>Reporte Mesh CLI Multiusuario \u2014 LinkGuard v2</h1>
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Genera reporte HTML para topologia mesh (4 nubes/vm + 1 local)")
    ...
    if args.output == str(REPORT_PATH):
        ...
        output_path = ROOT / "docs" / "reports" / "parrot" / "hub-spoke" / date_dir / f"reporte-mesh-{ts_slug}.html"
    else:
    ...
    try:
        result = run_topology_cycle("mesh", admin_token)

...
def _ssh_user(host: str) -> str:
    """Devuelve el usuario SSH para un host (cloud peer u orquestador)."""
    for p in REAL_PEERS:
        if p["host"] == host:
            return p.get("user", "root")
    return ORCH_USER if host == ORCH_HOST else "root"
...
def ssh_real(host: str, *remote_cmd: str, timeout: int = 120, check: bool = False) -> subprocess.CompletedProcess[str]:
    ...
    for _ in range(3):
        if _has_key_auth():
            ...
            if not REMOTE_PASS:
                raise RuntimeError("Define LINKGUARD_REMOTE_PASS o LINKGUARD_SSH_KEY")
            cmd = ["sshpass", "-p", REMOTE_PASS, "ssh", *SSH_OPTS, f"root@{host}"] + list(remote_cmd)
        last = subprocess.run(cmd, text=True, capture_output=True, timeout=timeout, check=False)
...
def ssh_real_script(host: str, script: str, *, timeout: int = 120, check: bool = False) -> subprocess.CompletedProcess[str]:
    ...
    for _ in range(3):
        if _has_key_auth():
            ...
            if not REMOTE_PASS:
                raise RuntimeError("Define LINKGUARD_REMOTE_PASS o LINKGUARD_SSH_KEY")
            cmd = ["sshpass", "-p", REMOTE_PASS, "ssh", *SSH_OPTS, f"root@{host}", "sh"]
        last = subprocess.run(
            cmd,
...
rm -f /etc/wireguard/wg0.conf /etc/wireguard/wg0.key /etc/wireguard/wg0.pub /etc/linkguard/wg-auto.json /etc/wireguard/wg-auto.json /etc/linkguard/peer.env
...
def _peer_env(peer_id: str) -> str:
    return f"""# LinkGuard Peer configuration - v2
ORCH_URL={REAL_ORCH_URL}
...
def run_topology_cycle(topology: str, admin_token: str) -> TopologyResult:
    ...
    if len(ip_map) >= 3:
        ...
        # Exclude terminal and rpi peers from iperf3 (needs internet)
        normal_clients = []
...
<title>Reporte Hub-Spoke CLI Multiusuario \u2014 LinkGuard v2</title>
<style>
...
<body>
<h1>Reporte Hub-Spoke CLI Multiusuario \u2014 LinkGuard v2</h1>
...
def main() -> int:
    ...
    if not REMOTE_PASS and not _has_key_auth():
        print("ERROR: Define LINKGUARD_REMOTE_PASS o TESTBED_REAL_ORCH_PASS (o LINKGUARD_SSH_KEY)", file=sys.stderr)
        return 1

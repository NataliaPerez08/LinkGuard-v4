#!/usr/bin/env python3
"""Genera reporte HTML via CLI con peer local (parrot) + nubes."""
...
REPORT_PATH = ROOT / "docs" / "reports" / "parrot" / "hub-mesh" / "latest.html"
...
SSH_KEY_PATH = os.environ.get("LINKGUARD_SSH_KEY", "/home/natalia/Downloads/linkguard-scripts/linkguard-key.pem")
...
REAL_PEERS = [
    {"peer_id": "peer-98-87-229-24", "host": "98.87.229.24", "label": "24 nube (98.87.229.24)", "user": "ec2-user"},
    {"peer_id": "peer-3-228-113-66", "host": "3.228.113.66", "label": "66 nube (3.228.113.66)", "user": "ec2-user"},
    {"peer_id": "peer-54-210-49-136", "host": "54.210.49.136", "label": "136 nube (54.210.49.136)", "user": "ubuntu"},
    {"peer_id": "peer-172-20-0-40", "host": TERMINAL_HOST, "label": "40 vm (172.20.0.40)", "is_terminal": True},
    {"peer_id": "peer-172-20-0-7", "host": RPI_HOST, "label": "7 rpi5 (172.20.0.7)", "is_rpi": True},
]
...
def _has_key_auth() -> bool:
    return bool(SSH_KEY_PATH) and os.path.exists(SSH_KEY_PATH)
...
def ssh_real(host: str, *remote_cmd: str, timeout: int = 120, check: bool = False) -> subprocess.CompletedProcess[str]:
    ...
    for _ in range(3):
        if _has_key_auth():
            user = _ssh_user(host)
            cmd = ["ssh", "-i", SSH_KEY_PATH, *SSH_OPTS, f"{user}@{host}"] + list(remote_cmd)
            last = run(cmd, timeout=timeout, check=False)
        elif REMOTE_PASS:
            last = run(["sshpass", "-p", REMOTE_PASS, "ssh", *SSH_OPTS, f"root@{host}", *remote_cmd], timeout=timeout, check=False)
        else:
            raise RuntimeError("Define LINKGUARD_SSH_KEY o LINKGUARD_REMOTE_PASS")
...
def ssh_real_script(host: str, script: str, *, timeout: int = 120, check: bool = False) -> subprocess.CompletedProcess[str]:
    ...
    for _ in range(3):
        if _has_key_auth():
            user = _ssh_user(host)
            cmd = ["ssh", "-i", SSH_KEY_PATH, *SSH_OPTS, f"{user}@{host}", "sudo", "sh"]
            last = subprocess.run(cmd, input=script, text=True, capture_output=True, timeout=timeout, check=False)
        elif REMOTE_PASS:
            last = subprocess.run(
...
def ssh_rpi(*remote_cmd: str, timeout: int = 120) -> subprocess.CompletedProcess[str]:
    return run(["sshpass", "-p", RPI_PASS, "ssh", *SSH_OPTS, "-J", TERMINAL_JUMP, f"{RPI_USER}@{RPI_HOST}", *remote_cmd], timeout=timeout, check=False)


def ssh_rpi_script(script: str, timeout: int = 120) -> subprocess.CompletedProcess[str]:
    last = None
    for _ in range(3):
        last = subprocess.run(
            ["sshpass", "-p", RPI_PASS, "ssh", *SSH_OPTS, "-J", TERMINAL_JUMP, f"{RPI_USER}@{RPI_HOST}", "sudo", "sh"],
            input=script, text=True, capture_output=True, timeout=timeout, check=False,
        )
        if last.returncode == 0:
            return last
...
_tenant_jwt = ""     # seteado por ensure_tenant_user()
_tenant_token = ""   # user token del tenant

def _parse_first_json(text: str) -> dict[str, Any]:
    """Extrae el primer objeto JSON del texto (auth-login agrega ayuda despues)."""
    text = text.strip()
    if not text:
        return {}
...
def orch_cli(*args: str, admin_token: str = "", token: str = "", timeout: int = 120) -> dict[str, Any]:
    """Ejecuta orch-cli.py localmente, con --admin-token o --token segun corresponda."""
    cmd = ["python3", ORCH_CLI, "--url", REAL_ORCH_URL]
    if admin_token:
        cmd.extend(["--admin-token", admin_token])
    if token:
    ...
    if not out:
        return {}
    parsed = _parse_first_json(out)
    if parsed:
        return parsed
    # Fallback: texto plano (health devuelve JSON multilinea a veces)
    return {"_raw": out}
...
def get_admin_token() -> str:
    # Use orch-cli directly with the known admin token
    return "d8fb642ea0c59b3c0b4995ce834cbdaabd9be49ac3328116790c914da74afbe3"
...
def ensure_tenant_user() -> str:
    """Crea usuario tenant, obtiene su token y JWT. Retorna el JWT."""
...
def peer_unregister(peer_id: str, admin_token: str) -> bool:
    """peer.unregister via CLI usando JWT del tenant."""
    try:
...
def _peer_env(peer_id: str) -> str:
    return f"""ORCH_URL={REAL_ORCH_URL}
ORCH_TOKEN={_tenant_token}
PEER_ID={peer_id}
USER_ID={TENANT_USER}
WG_INTERFACE=wg0
...
def clean_rpi_peer() -> None:
    script = """set -e
sudo systemctl stop wg-auto-register 2>/dev/null || true
sudo killall -9 wg-auto-register.py 2>/dev/null || true
sudo rm -f /etc/wireguard/wg0.conf /etc/wireguard/wg0.key /etc/wireguard/wg0.pub /etc/linkguard/wg-auto.json /etc/wireguard/wg-auto.json
sudo rm -f /etc/linkguard/peer.env
sudo ip link delete wg0 2>/dev/null || true
"""
    ssh_rpi_script(script, timeout=60)


def start_rpi_peer() -> None:
    ssh_rpi_script("""set -e
sudo systemctl stop wg-auto-register 2>/dev/null || true
sudo killall -9 wg-auto-register.py 2>/dev/null || true
...
def start_all_peers() -> None:
    log("Escribiendo peer.env en peers reales...")
    for p in REAL_PEERS:
        env = _peer_env(p["peer_id"])
        if p.get("is_rpi"):
            ssh_rpi_script(f"cat > /etc/linkguard/peer.env << 'PEEREOF'\n{env}\nPEEREOF", timeout=30)
        elif p.get("is_terminal"):
            ssh_terminal_script(f"cat > /etc/linkguard/peer.env << 'PEEREOF'\n{env}\nPEEREOF", timeout=30)
        else:
            ssh_real_script(p["host"], f"cat > /etc/linkguard/peer.env << 'PEEREOF'\n{env}\nPEEREOF", timeout=30)
        log(f"  {p['host']} peer.env escrito")
    ...
    local_script(f"cat > /etc/linkguard/peer.env << 'PEEREOF'\n{_peer_env(LOCAL_PEER['peer_id'])}\nPEEREOF", timeout=30)
    start_local_peer()
    log("  parrot iniciado")
...
def ping_rpi(target: str, count: int = 5) -> PingResult:
    result = ssh_rpi("ping", "-n", "-c", str(count), "-W", "2", target, timeout=180)
    return parse_ping_output((result.stdout or "") + (result.stderr or ""))
...
def capture_wg_configs(peers: dict[str, Any]) -> dict[str, str]:
    ...
    for p in REAL_PEERS:
        try:
            if p.get("is_rpi"):
                r = ssh_rpi("wg", "show", "wg0", timeout=30)
                configs[p["peer_id"]] = r.stdout or "(no output)"
                r2 = ssh_rpi("wg", "showconf", "wg0", timeout=30)
...
def run_topology_cycle(topology: str, admin_token: str) -> TopologyResult:
        ...
        for dst_id in ALL_PEER_IDS:
            ...
            try:
                if src_real:
                    p = src_real[0]
                    if p.get("is_rpi"):
                        ping_results[src_id][dst_id] = ping_rpi(dst_ip)
                    elif p.get("is_terminal"):
    ...
    if len(ip_map) >= 3:
        ...
        for p in REAL_PEERS[1:]:
            if p.get("is_terminal") or p.get("is_rpi"):
                continue
    ...
    log("Fase 8: Midiendo throughput local (parrot)...")
...
def _path_type(src_id: str, dst_id: str, topology: str = "hub-mesh") -> str:
...
<title>Reporte Hub-Mesh CLI Multiusuario \u2014 LinkGuard v2</title>
<style>
...
<body>
<h1>Reporte Hub-Mesh CLI Multiusuario \u2014 LinkGuard v2</h1>
<p>Generado: <code>""" + timestamp + """</code> | Orquestador: <code>""" + ORCH_HOST + """</code></p>
<p>Peers: """ + str(len(REAL_PEERS)) + """ nube/vm + 1 local (total """ + str(len(ALL_PEER_IDS)) + """)</p>
""")
    ...
    for pid in sorted(peers_inventory):
        ...
        tag_cls = {"NUBE": "tag-nube", "LOCAL": "tag-local", "VM": "tag-vm"}.get(ptype, "tag-nube")
    ...
    rows.append('<div class="section" id="auth-info"><h2>Modo de autenticacion CLI</h2>')
    ...
    rows.append(f'</table>')
    rows.append(f'<p class="legend">Usuario tenant: <code>{TENANT_USER}</code> | JWT generado via <code>auth-login</code></p>')
    rows.append('</div>')
    ...
    for r in results:
        ...
        if failed:
            ...
            if dst_terminal:
                rows.append(f'<p><b>Terminal (172.20.0.40):</b> Esta VM esta detras de un jump-host y podria no tener conectividad directa al orquestador ({ORCH_HOST}). Verificar ruta de red y NAT en el jump-host 100.115.215.49.</p>')
            if dst_local:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Genera reporte HTML para topologia hub-mesh (4 nubes/vm + 1 local)")
    parser.add_argument("--output", default=str(REPORT_PATH))
    parser.add_argument("--skip-throughput", action="store_true")
    parser.add_argument("--skip-clean", action="store_true")
    args = parser.parse_args()

    if not REMOTE_PASS and not _has_key_auth():
        print("ERROR: Define LINKGUARD_SSH_KEY o LINKGUARD_REMOTE_PASS", file=sys.stderr)
        return 1
    ...
    if args.output == str(REPORT_PATH):
        ...
        output_path = ROOT / "docs" / "reports" / "parrot" / "hub-mesh" / date_dir / f"reporte-hubmesh-{ts_slug}.html"
    else:
    ...
    try:
        result = run_topology_cycle("hub-mesh", admin_token)

#!/usr/bin/env python3
"""Modulo comun para tests AWS Multiusuario CLI con CIDR alternativo (1 QEMU).

Combina:
  - Infraestructura AWS (3 EC2 + 1 VM + 1 RPi5 + 1 QEMU)
  - Autenticacion multiusuario (admin + tenant-a) via orch-cli
  - Reconexion a CIDR alternativo (10.99.88.0/24)
...
def reconfigure_orchestrator_cidr() -> None:
    log(f"Reconfigurando orquestador a CIDR alternativo {ALT_CIDR}...")
...
sleep 5
"""
    ssh_orch_sudo_script(script, timeout=180, check=True)
...
def restore_orchestrator_cidr() -> None:
    log("Restaurando CIDR original 10.20.30.0/24...")
    script = """set -e
...
"""
    try:
        ...
        ssh_orch_sudo_script("systemctl start hub-agent orchestrator 2>/dev/null || true", timeout=30)
...
def render_html(title: str, timestamp: str, results: list[TopologyResult], alt_cidr: bool = False) -> str:
    ...
    rows: list[str] = []
    cidr_label = f" CIDR {ALT_CIDR}" if alt_cidr else ""
    hub_ip = ALT_HUB_IP if alt_cidr else "10.20.30.1"
    cidr_str = ALT_CIDR if alt_cidr else "10.20.30.0/24"
    rows.append("""<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>""" + title + cidr_label + """</title>
<style>
...
<body>
<h1>""" + title + cidr_label + """</h1>
<p>Generado: <code>""" + timestamp + """</code> | Orquestador: <code>""" + ORCH_HOST + """</code> (usuario <code>""" + ORCH_USER + """</code>)</p>
<p>Peers: """ + str(len(REAL_PEERS)) + """ reales + """ + str(len(QEMU_PEERS)) + """ QEMU (total """ + str(len(ALL_PEER_IDS)) + """) | CIDR: <code>""" + cidr_str + """</code></p>
""")
    ...
    rows.append(f'</table><p class="legend">Hub: <code>{ORCH_HOST}:51820</code> \u2014 IP tunel: <code>{hub_ip}</code> | CIDR: <code>{cidr_str}</code> | SSH nubes: llave <code>linkguard-key.pem</code> | VM/RPi5 via jump <code>{TERMINAL_JUMP}</code></p></div>')
    for r in results:
    ...
    rows.append(f'</table>')
    rows.append(f'<p class="legend">Usuario tenant: <code>{TENANT_USER}</code> | JWT via <code>auth-login</code> | CIDR: <code>{cidr_str}</code></p>')
    rows.append('</div>')
    ...
    if alt_cidr:
        ...
        for r in results:
            ...
            if partial:
                rows.append(f'<p>Perdida parcial en {len(partial)} pares. Causa probable: congestion del hub como relay bajo trafico concurrente.</p>')
            if failed:
                dst_qemu = [d for s, d, p in failed if d in [q["peer_id"] for q in QEMU_PEERS]]
                if dst_qemu:
                    rows.append('<p><b>Fallos hacia QEMU \u2014 NAT simetrico:</b> El peer QEMU esta detras del host local con NAT de puerto dinamico. El hub conoce el endpoint NAT del QEMU, pero al iniciar handshake el NAT asigna un puerto diferente y el QEMU nunca recibe el paquete.</p>')
...
def write_jsonl_report(timestamp: str, results: list[TopologyResult], alt_cidr: bool = False) -> None:
    filename = "aws-multiuser-cli-1qemu-altcidr-historial.jsonl"
    jsonl_path = ROOT / "docs" / "reports" / filename
    ...
    for r in results:
        ...
        record = {
            ...
            "topology": r.topology,
            "cidr": ALT_CIDR if alt_cidr else "10.20.30.0/24",
...
def run_report(topology: str, *, title: str, report_subdir: str, file_prefix: str, default_topology: str = "hub-spoke", description: str = "") -> int:
    ...
    parser.add_argument("--alt-cidr", action="store_true", help=f"Usa CIDR alternativo {ALT_CIDR}")
    ...
    if args.output:
        ...
        base_dir = "aws-multiuser-cli-1qemu-altcidr"
        output_path = ROOT / "docs" / "reports" / base_dir / report_subdir / date_dir / f"{file_prefix}-{ts_slug}.html"
    ...
    alt_cidr = args.alt_cidr
    if alt_cidr:
        reconfigure_orchestrator_cidr()
        log("Usando CIDR alternativo para todas las topologias")
    ...
    try:
        ...
        html = render_html(title, timestamp, results, alt_cidr=alt_cidr)
        output_path.write_text(html)
        log(f"Reporte generado en {output_path}")
        write_jsonl_report(timestamp, results, alt_cidr=alt_cidr)
    except Exception as exc:
        log(f"ERROR: {exc}")
        import traceback
        traceback.print_exc()

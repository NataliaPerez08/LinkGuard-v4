#!/usr/bin/env python3
"""Genera reporte HTML para topologia mesh con CIDR alternativo (10.99.88.0/24) y 4 nubes/vm + 3 QEMU."""
...
REPORT_PATH = ROOT / "docs" / "reports" / "10.99.88.0_alt" / "mesh" / "latest.html"
...
def _orch_script(script: str, *, timeout: int = 120) -> subprocess.CompletedProcess[str]:
    return ssh_real_script(ORCH_HOST, script, timeout=timeout, check=True)


def reconfigure_orchestrator_cidr(new_cidr: str, new_hub_ip: str) -> None:
    log(f"Eliminando red 'default' actual del orquestador...")
    try:
        r = rpc().__getattr__("network.delete")("default")
        log(f"  network.delete: {r}")
    except Exception as exc:
        log(f"  WARN: network.delete: {exc}")

    script = f"""set -e
systemctl stop orchestrator hub-agent
sleep 3
# Tear down old WG interface and delete conf so hub-agent regenerates it
...
# Orchestrator: ensure DEFAULT_NET_CIDR is set + update WG_TUNNEL_CIDR
...
# Hub-agent: update HUB_TUNNEL_IP and HUB_TUNNEL_CIDR
...
sleep 5
# Clean up default network so it gets recreated with new CIDR
# (auto_approve will recreate it on first peer registration)
"""
    _orch_script(script, timeout=180)
    log(f"Orquestador reconfigurado para CIDR {new_cidr} / hub IP {new_hub_ip}")
    time.sleep(5)
    try:
        r = rpc().__getattr__("network.create")("default", new_cidr)
        log(f"  network.create 'default': {r}")
    except Exception as exc:
        log(f"  WARN: network.create 'default': {exc}")
    try:
        r = rpc().__getattr__("orch.health")()
        log(f"  Health check: {r.get('status', 'unknown')}")
    except Exception as exc:
        log(f"  WARN: health check despues de reconfigure: {exc}")


def restore_orchestrator_cidr(admin_token: str) -> None:
    script = """set -e
systemctl stop orchestrator hub-agent 2>/dev/null || true
sleep 3
# Tear down WG interface + conf so hub-agent regenerates with restored CIDR
wg-quick down wg-HUB 2>/dev/null || ip link delete wg-HUB 2>/dev/null || true
rm -f /etc/wireguard/wg-HUB.conf
...
"""
    ...
    time.sleep(5)
    # Delete residual default network (alt-CIDR) and recreate with original CIDR
    try:
        rpc().__getattr__("network.delete")("default")
        log("  Red 'default' residual eliminada")
    except Exception as exc:
        log(f"  WARN: network.delete 'default': {exc}")
    try:
        r = rpc().__getattr__("network.create")("default", "10.20.30.0/24")
        log(f"  Red 'default' recreada con CIDR 10.20.30.0/24: {r}")
    except Exception as exc:
        log(f"  WARN: network.create 'default': {exc}")
    # Clean all peers from orchestrator + WG hub
    clean_orchestrator_peers(admin_token)
    hub_flush_peers(admin_token)
    # Clean WG on all peers and restart them so they re-register with original CIDR
...
<title>Reporte Mesh \u2014 CIDR """ + ALT_CIDR + """ \u2014 LinkGuard v2</title>
<style>
...
<body>
<h1>Reporte Mesh \u2014 CIDR """ + ALT_CIDR + """ \u2014 LinkGuard v2</h1>
<p>Generado: <code>""" + timestamp + """</code> | Orquestador: <code>""" + ORCH_HOST + """</code> | CIDR: <code>""" + ALT_CIDR + """</code></p>
...
""")
    ...
    rows.append(f'</table><p class="legend">Hub: <code>{ORCH_HOST}:51820</code> \u2014 IP tunel: <code>{ALT_HUB_IP}</code> | CIDR: <code>{ALT_CIDR}</code></p></div>')
    for r in results:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Genera reporte HTML para topologia mesh (4 nubes/vm + 3 QEMU)")
    parser.add_argument("--output", default=str(REPORT_PATH))
    parser.add_argument("--skip-qemu", action="store_true")
    parser.add_argument("--skip-throughput", action="store_true")
    parser.add_argument("--skip-clean", action="store_true")
    parser.add_argument("--no-restore", action="store_true", help="No restaurar CIDR original ni limpiar peers al terminar")
    ...
    if args.output == str(REPORT_PATH):
        ...
        output_path = ROOT / "docs" / "reports" / "10.99.88.0_alt" / "mesh" / date_dir / f"reporte-mesh-cidr{CIDR_SLUG}-{ts_slug}.html"
    else:
    ...
    reconfigure_orchestrator_cidr(ALT_CIDR, ALT_HUB_IP)
    ...
    try:
        ...
        if args.no_restore:
            ...
            log(f"Para restaurar manualmente ejecuta: LINKGUARD_REMOTE_PASS='...' python3 {__file__} --skip-qemu --skip-throughput --skip-clean --output /dev/null")
        else:
            log("Restaurando orquestador a CIDR original...")
            restore_orchestrator_cidr(admin_token)
    except Exception as exc:
        log(f"ERROR: {exc}")
        import traceback
        traceback.print_exc()

...
def _update_peer_env_token(host: str) -> None:
    for p in REAL_PEERS:
        ...
        peer_id = p["peer_id"]
        script = (
            f"sed -i 's|^ORCH_TOKEN=.*|ORCH_TOKEN={_tenant_token}|' /etc/linkguard/peer.env; "
            f"sed -i 's|^USER_ID=.*|USER_ID={TENANT_USER}|' /etc/linkguard/peer.env; "
            f"sed -i 's|^PEER_ID=.*|PEER_ID={peer_id}|' /etc/linkguard/peer.env; "
            f"grep -q '^USER_ID=' /etc/linkguard/peer.env || echo 'USER_ID={TENANT_USER}' >> /etc/linkguard/peer.env; "
...
def write_jsonl_report(timestamp: str, results: list[TopologyResult]) -> None:
    jsonl_path = ROOT / "docs" / "reports" / "aws-multiuser-cli-1qemu-historial.jsonl"
...
def run_report(topology: str, *, title: str, report_subdir: str, file_prefix: str, default_topology: str = "hub-spoke", description: str = "") -> int:
    ...
    if args.output:
        ...
        output_path = ROOT / "docs" / "reports" / "aws-multiuser-cli-1qemu" / report_subdir / date_dir / f"{file_prefix}-{ts_slug}.html"

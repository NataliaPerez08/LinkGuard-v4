...
HUB_AGENT_PORT    = int(os.getenv("HUB_AGENT_PORT", "9000"))
...
HUB_LISTEN_PORT   = int(os.getenv("HUB_LISTEN_PORT", "51820"))
...
def ensure_keys():
    ...
    priv = run([WG_BIN, "genkey"], capture=True).strip()
    pub  = run(["bash", "-lc", f"echo '{priv}' | {WG_BIN} pubkey"], capture=True).strip()
...
def cleanup_iptables_rules():
    ...
    for args in rules_to_delete:
        try:
            subprocess.run([IPTABLES_BIN] + args, check=False,
...
def ensure_base_rules_internal():
    ensure_wg_up()
    ensure_ip_forward()
    ensure_forward_rule()
    ensure_nat_rules()
...
# XML-RPC ENDPOINTS
# =========================
def hub_health(token: str):
    check_token(token)
    return "ok"


def hub_apply_peer(token: str, public_key: str, allowed_ips: str):
    check_token(token)
    ensure_wg_up()

    if HUB_MESH_MODE == "mesh":
        # mesh puro (v3): el HUB solo necesita la IP de túnel del peer (/32)
        # Filtrar: conservar /32, descartar CIDRs amplios
        parts = [ip.strip() for ip in allowed_ips.split(",") if ip.strip()]
        mesh_parts = [ip for ip in parts if ip.endswith("/32")]
        if not mesh_parts:
            mesh_parts = parts  # fallback
        allowed_ips = ",".join(mesh_parts)
        log(f"[MESH] peer {public_key[:16]}... → {allowed_ips}")

    elif HUB_MESH_MODE == "hub-mesh":
        # hub-mesh (v4): el HUB retiene todo lo que llega desde el orchestrator
        # (el orchestrator ya pasó el CIDR correcto; no filtrar)
        log(f"[HUB-MESH] peer {public_key[:16]}... → {allowed_ips} (relay activo)")

    else:
        # off: hub-spoke clásico
        log(f"Aplicando peer en {HUB_WG_IFACE}: {public_key[:16]}... → {allowed_ips}")

    run([WG_BIN, "set", HUB_WG_IFACE, "peer", public_key, "allowed-ips", allowed_ips], check=True)
    return True


def hub_remove_peer(token: str, public_key: str):
    check_token(token)
    ensure_wg_up()
...
def hub_ensure_base_rules(token: str):
    check_token(token)
    ensure_base_rules_internal()
    return True


def hub_show(token: str):
    check_token(token)
    ensure_wg_up()
    return run([WG_BIN, "show", HUB_WG_IFACE], capture=True)


def hub_public_key(token: str):
    check_token(token)
    try:
        return subprocess.check_output([WG_BIN, "show", HUB_WG_IFACE, "public-key"], text=True).strip()
    except subprocess.CalledProcessError as e:
        raise Exception(f"wg failed: {e}")


# OPS-01: endpoint para llamar desde systemd ExecStop
def hub_cleanup(token: str):
    check_token(token)
...
def hub_list_mesh_peers(token: str, network_cidr: str = "") -> list:
    ...
    try:
        ...
        raise Exception(f"wg show dump failed: {e}")
    ...
    for line in out.splitlines()[1:]:  # saltar línea de la interfaz
        ...
        if network_cidr:
            ...
            try:
                net = _ip.IPv4Network(network_cidr, strict=False)
                ips_in_net = [
                    a.split("/")[0] for a in allowed_ips_raw.split(",")
                    if _ip.IPv4Address(a.split("/")[0]) in net
                ]
                if not ips_in_net:
                    continue
            except Exception:
